...
const Profile = ({ user, setAlert, rawRequests }) => {
  ...
  const requests = [
    { label: "To Pick up", content: `${status[0]} ${status[0] == 1 ? 'entry' : 'entries'}` },
    { label: "Completed", content: `${status[1]} ${status[1] == 1 ? 'entry' : 'entries'}` },
    { label: "Cancelled", content: `${status[2]} ${status[2] == 1 ? 'entry' : 'entries'}` },
        ...
        <div className='relative flex flex-row'>
          ...
          <div className='flex flex-col border-2 border-[#1F2F3D] bg-[#D9D9D9] w-48 h-auto rounded-lg'>
            ...
            <div className='border-b-2 border-[#1F2F3D]'>
              <h2 className='text-lg text-center font-bold my-2'>REQUESTS</h2>
            </div>
