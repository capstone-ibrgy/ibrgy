...
function UserDashboard({ profile, setScreen, documents }) {
  ...
  const getDocumentName = (form) => {
    return documents["documents"].map((item) => {
  ...
  return (
    <>
      <h1 className="py-4 text-3xl font-bold">
        Welcome back,{" "}
        <span className="text-[#1B75BC]">{profile.firstname}</span>
      </h1>
        ...
        <div className="flex flex-col w-[30%] h-full bg-[#FEC51C] rounded-[20px]">
          <div className="h-12 flex flex-row px-5 items-center">
            ...
            <select
              name="show"
              id="show"
              className="bg-[#FEC51C] border-2 border-[#1F2F3D]"
            >
                      ...
                      <div key={`en#${i}`} className="pb-4 py-2 px-6 border-b">
                        ...
                        {entries[date].map((form, i) => {
                          return (
                            <ul
                              key={`li#${i}`}
                              className="px-4 list-disc list-inside"
                            >
                              <li className="text-sm">
                                {getDocumentName(form)}
                              </li>
                            </ul>
                          );
                        })}
