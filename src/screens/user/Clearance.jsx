...
import { ClearanceForm } from '../../models/ClearanceForm'
import {
  requestForm,
  ref,
  storage,
...
const Clearance = ({ height, profile, setProgress, setShowUpload }) => {

  const [startDate, setStartDate] = useState();
  const [fileError, setFileError] = useState('');
  const { form, updateForm } = ClearanceForm();
      ...
      <div
        ...
        <div
          ...
          <div className='relative flex flex-row w-full h-16 bg-[#1F2F3D] rounded-[20px] items-center px-6 gap-6'>
            ...
            <h1 className='text-white text-xl font-bold'>{data['documents'][1].name}</h1>
            <div className='absolute right-[-1px] rounded-[20px] w-28 h-full bg-[#FEC51C] py-3 px-6'>
              <h1 className='font-bold text-lg text-center leading-tight'>COST: {data['documents'][1].price} Php</h1>
            </div>
          </div>
          <form onSubmit={handleSubmit} className='flex flex-col w-full flex-1 px-6 py-2 gap-1'>
            <div className='flex-1 flex flex-col w-1/3'>
              <label className='py-1 font-bold text-sm'>Nickname</label>
              <input value={form.nickname} required={true} onChange={(e) => { updateForm({ nickname: e.target.value }) }} className='border-2 h-9 border-[#1F2F3D] bg-[#D9D9D9] rounded-[10px] px-2' />
            </div>
