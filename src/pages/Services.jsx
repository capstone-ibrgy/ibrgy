                  ...
                  <div className={`h-20 w-full bg-white ${dropdown === i && 'rounded-b-[20px] '}`}>
                    <div className='relative flex flex-row h-full rounded-[20px] bg-[#FEC51C]'>
                      <div className='flex w-20 items-center justify-center bg-[#FEC51C] rounded-l-[20px]'>
                        <img onClick={() => { handleDropdown(i) }}
                          className={`w-6 h-6 cursor-pointer duration-500 transition-all ease-in-out  
                        ${dropdown === i ? 'rotate-90' : 'rotate-0'}`} src={arrow} alt='arrow' />
                      </div>
                      ...
                      <div onClick={() => { handleRequest(i) }} className='cursor-pointer flex justify-center items-center z-10 h-full right-0 absolute w-40 bg-[#FEC51C] rounded-[20px] drop-shadow-lg'>
